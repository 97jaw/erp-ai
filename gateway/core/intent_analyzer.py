...
class IntentAnalyzer:
    ...
    def _build_prompt(self, query: str, context: ContextStack) -> str:
        ...
        return (
            ...
            'list_records|search_entity|explain|ask_question|other",'
            ...
            "}\n"
            "LIST AND CROSS-LIST QUERIES (CRITICAL — handle these correctly):\n"
            "If the user is asking for a LIST of records (not deep analysis of one specific record), "
            "set primary_action = 'list_records'.\n"
            "LIST patterns (single model):\n"
            "  'list X' → list_records of X\n"
            "  'show all X' → list_records of X\n"
            "  'active X' → list_records with filter\n"
            "  'X of [year/period]' → list_records with date filter\n"
            "  'which X are [condition]' → list_records with filter\n"
            "  'X grouped by Y' → list_records with group_by\n"
            "  'X by Y' / 'Y wise X' → list_records with group_by Y\n"
            "  'top N X by Z' → list_records ordered by Z\n"
            "CROSS-LIST patterns (entity A filtered by entity B):\n"
            "  'X for Y' → cross-list (list X where it relates to Y)\n"
            "  'X assigned to Y' → cross-list\n"
            "  'X of Y' (when Y is entity) → cross-list\n"
            "  'X without Y' → inverse cross-list (anti-join)\n"
            "  'X with no Y' → inverse cross-list\n"
            "Examples (set primary_action=list_records for ALL of these):\n"
            "  'list projects of 2024'\n"
            "    primary_action: list_records, subject_area: project\n"
            "    entities: [{type:'date_filter', value:'2024'}]\n"
            "  'active projects of 2026'\n"
            "    primary_action: list_records, subject_area: project\n"
            "    entities: [{type:'date_filter', value:'2026'}, {type:'status_filter', value:'active'}]\n"
            "  'client wise projects of 2024'\n"
            "    primary_action: list_records, subject_area: project\n"
            "    entities: [{type:'date_filter', value:'2024'}, {type:'group_by', value:'partner_id'}]\n"
            "  'employees on Villa 34'\n"
            "    primary_action: list_records, subject_area: hr\n"
            "    entities: [{type:'related_entity', value:'Villa 34', relation:'project_id'}]\n"
            "  'vehicles assigned to Adil Khan'\n"
            "    primary_action: list_records, subject_area: fleet\n"
            "    entities: [{type:'related_entity', value:'Adil Khan', relation:'driver_or_employee'}]\n"
            "  'POs for Villa 34'\n"
            "    primary_action: list_records, subject_area: purchase\n"
            "    entities: [{type:'related_entity', value:'Villa 34', relation:'project_or_analytic'}]\n"
            "  'invoices for Abu Dhabi Police'\n"
            "    primary_action: list_records, subject_area: financial\n"
            "    entities: [{type:'related_entity', value:'Abu Dhabi Police', relation:'partner_id'}]\n"
            "  'projects with no attachments'\n"
            "    primary_action: list_records, subject_area: project\n"
            "    entities: [{type:'anti_join', value:'project.attachment'}]\n"
            "  'employees without contracts'\n"
            "    primary_action: list_records, subject_area: hr\n"
            "    entities: [{type:'anti_join', value:'hr.contract'}]\n"
            "CRITICAL RULES FOR LIST QUERIES:\n"
            "  1. DO NOT extract the model being listed as a single entity. "
            "'projects of 2024' → projects is the MODEL, 2024 is the FILTER.\n"
            "  2. DO NOT set requires_clarification=true for list queries. "
            "'list employees' is complete, not ambiguous.\n"
            "  3. NEVER assume list queries are about single records needing confirmation. "
            "List = many records.\n"
            "  4. DEEP-ANALYSIS vs LIST:\n"
            "     'Villa 34 expense' → single-entity, fetch_data\n"
            "     'list projects' → list_records\n"
            "     'P&L this year' → single-report, fetch_data\n"
            "     'compare A and B' → compare (not list_records)\n"
            "     'top 5 by expense' → list_records (still a list)\n"
